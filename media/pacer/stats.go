...
type OutStatsPeriod struct {
	...
	JBD        statsutil.RawStatistics[duration.Millis] `json:"jbd"`        // jitter buffer delay stats for last period
...
type OutStats struct {
	...
	jbd        statsutil.Periodic[duration.Millis] // collector for jitter buffer delays
...
func newOutStats(period duration.Duration) OutStats {
	...
	return OutStats{
		...
		blocked:    statsutil.Periodic[duration.Millis]{ManualSwitch: true, Period: specPeriod},
	}
}
...
// UpdateJBD records a jitter buffer delay (time from packet arrival to delivery) sample. Must be called under
// outStatsMu.
func (s *OutStats) UpdateJBD(now utc.UTC, inTs utc.UTC) {
	s.jbd.UpdateNow(now, duration.Millis(now.Sub(inTs)))
}
