...
func TestParseLiveUrlUnicast(t *testing.T) {
	...
	liveURL, err := ParseLiveUrlString(url)
...
func TestParseLiveUrlLoopback(t *testing.T) {
	{
		liveURL, err := ParseLiveUrlString("udp://232.1.2.3:5000?loopback")
		assert.NoError(t, err)
		assert.True(t, liveURL.Loopback)
	}
	{
		liveURL, err := ParseLiveUrlString("udp://232.1.2.3:5000")
		assert.NoError(t, err)
		assert.False(t, liveURL.Loopback)
	}
}
