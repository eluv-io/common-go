...
func New(htype Type, digest []byte, size int64, id ei.ID) (*Hash, error) {
	...
	if size < 0 {
		return nil, errors.E("init hash", errors.K.Invalid, "reason", "invalid size", "size", size)
	}

	if htype.Code == Q && id.AssertCode(ei.Q) != nil {
		return nil, errors.E("init hash", errors.K.Invalid, "reason", "invalid id", "id", id)
	}
...
func FromString(s string) (*Hash, error) {
	...
	if len(s) <= prefixLen {
		return nil, errors.E("parse hash", errors.K.Invalid, "reason", "invalid string", "string", s)
	}

	htype, found := prefixToType[s[:prefixLen]]
	if !found {
		return nil, errors.E("parse hash", errors.K.Invalid, "reason", "invalid prefix", "string", s)
	}

	b, err := base58.Decode(s[prefixLen:])
	if err != nil {
		return nil, errors.E("parse hash", errors.K.Invalid, err, "string", s)
	}
	n := 0
