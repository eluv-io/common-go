...
func init() {
	...
	hsh = &hash.Hash{Type: htype, Digest: digest, Size: size, ID: id}
}
...
func TestCreation(t *testing.T) {
	payload, _ := hex.DecodeString("9cbc07c3f991725836a3aa2a581ca2029198aa420b9d99bc0e131d9f3e2cbe47")
	id, _ := id.FromString("iq__WxoChT9EZU2PRdTdNU7Ldf")
	h, err := hash.New(hash.Type{hash.Q, hash.Unencrypted}, payload, 1024, id)
	assert.NoError(t, err)
	assertHash(t, h, "hq_")
...
func TestDigest(t *testing.T) {
	...
	d := hash.NewDigest(sha256.New(), hash.Type{hash.Q, hash.Unencrypted}, id)
	b := make([]byte, 1024)
